import { MapPinIcon, PackageCheck, UserCircle, Users } from "lucide-react";
...
export function CardInformations({ address, residentAmount, deliveryAmount }) {
  return (
      ...
      <div className="w-full sm:w-80 h-full border rounded-lg shadow-lg">
        ...
        <div className="flex mb-2 px-8 pt-4 items-start gap-4">
          <p className="flex space-x-5 cursor-pointer">
            {[...Array(5)].map((_, i) => (
              <UserCircle key={i} size={24} color="#0074d9" />
            ))}
          </p>
        </div>
      </div>
